...
@section('title', 'Edit Data IPM')
@section('page-title', 'Edit Data IPM')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_ipm.update', $data_ipm->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="kecamatan" class="block text-sm font-medium text-gray-700">Nama Kecamatan</label>
                <input type="text" name="kecamatan" id="kecamatan" value="{{ old('kecamatan', $data_ipm->kecamatan) }}"
                    required
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="skor_ipm" id="skor_ipm"
                    value="{{ old('skor_ipm', $data_ipm->skor_ipm) }}" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('skor_ipm')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $data_ipm->tahun) }}" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('tahun')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
