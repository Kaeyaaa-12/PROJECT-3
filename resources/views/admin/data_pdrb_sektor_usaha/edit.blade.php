...
@section('title', 'Edit Data PDRB per Sektor Usaha')
@section('page-title', 'Edit Data PDRB')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_pdrb_sektor_usaha.update', $data_pdrb_sektor_usaha->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="tahun" class="block text-sm font-medium text-gray-700">Tahun</label>
                <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $data_pdrb_sektor_usaha->tahun) }}"
                    required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="pertanian" id="pertanian"
                        value="{{ old('pertanian', $data_pdrb_sektor_usaha->pertanian) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                <div>
                    <label for="industri" class="block text-sm font-medium text-gray-700">Industri (%)</label>
                    <input type="number" step="0.01" name="industri" id="industri"
                        value="{{ old('industri', $data_pdrb_sektor_usaha->industri) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="perdagangan" id="perdagangan"
                        value="{{ old('perdagangan', $data_pdrb_sektor_usaha->perdagangan) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                <div>
                    <label for="konstruksi" class="block text-sm font-medium text-gray-700">Konstruksi (%)</label>
                    <input type="number" step="0.01" name="konstruksi" id="konstruksi"
                        value="{{ old('konstruksi', $data_pdrb_sektor_usaha->konstruksi) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                <div>
                    <label for="jasa" class="block text-sm font-medium text-gray-700">Jasa (%)</label>
                    <input type="number" step="0.01" name="jasa" id="jasa"
                        value="{{ old('jasa', $data_pdrb_sektor_usaha->jasa) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
            </div>
