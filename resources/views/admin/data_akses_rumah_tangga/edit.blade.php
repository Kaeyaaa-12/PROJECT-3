...
@section('title', 'Edit Data Akses Rumah Tangga')
@section('page-title', 'Edit Data')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_akses_rumah_tangga.update', $data_akses_rumah_tangga->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="tahun" class="block text-sm font-medium text-gray-700">Tahun</label>
                <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $data_akses_rumah_tangga->tahun) }}"
                    required
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="persentase_akses_air_bersih_layak"
                        id="persentase_akses_air_bersih_layak"
                        value="{{ old('persentase_akses_air_bersih_layak', $data_akses_rumah_tangga->persentase_akses_air_bersih_layak) }}"
                        required
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="akses_sanitasi_layak" id="akses_sanitasi_layak"
                        value="{{ old('akses_sanitasi_layak', $data_akses_rumah_tangga->akses_sanitasi_layak) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="rasio_elektrifikasi" id="rasio_elektrifikasi"
                        value="{{ old('rasio_elektrifikasi', $data_akses_rumah_tangga->rasio_elektrifikasi) }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
            </div>
