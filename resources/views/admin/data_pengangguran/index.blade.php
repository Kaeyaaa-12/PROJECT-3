...
@section('page-title', 'Manajemen Data Pengangguran')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <a href="{{ route('admin.data_pengangguran.create') }}"
            class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 mb-6">
            <i class="ri-add-line mr-2"></i>Tambah Data Pengangguran
        </a>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border">
                ...
                <tbody class="text-gray-700">
                    @forelse($dataPengangguran as $data)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="py-3 px-4">{{ $loop->iteration }}</td>
                            <td class="py-3 px-4">{{ $data->kecamatan }}</td>
                            <td class="py-3 px-4 text-right">{{ number_format($data->persentase_pengangguran, 2) }}</td>
                            <td class="py-3 px-4 text-center">{{ $data->tahun }}</td>
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('admin.data_pengangguran.edit', $data->id) }}"
                                    class="text-yellow-500 hover:text-yellow-700 mr-4">
                                    <i class="ri-pencil-line"></i> Edit
                                </a>
                                <form action="{{ route('admin.data_pengangguran.destroy', $data->id) }}" method="POST"
                                    class="inline-block"
