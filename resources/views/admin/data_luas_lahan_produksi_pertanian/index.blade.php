...
@section('title', 'Data Luas Lahan & Produksi Pertanian')
@section('page-title', 'Manajemen Data Luas Lahan & Produksi Pertanian')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <a href="{{ route('admin.data_luas_lahan_produksi_pertanian.create') }}"
                ...
                <tbody class="text-gray-700">
                    @forelse($dataPertanian as $data)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="py-3 px-4">{{ $data->komoditas }}</td>
                            <td class="py-3 px-4 text-right">{{ number_format($data->luas_lahan, 2, ',', '.') }}</td>
                            <td class="py-3 px-4 text-right">{{ number_format($data->produksi, 2, ',', '.') }}</td>
                            <td class="py-3 px-4 text-center">{{ $data->tahun }}</td>
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('admin.data_luas_lahan_produksi_pertanian.edit', $data->id) }}"
                                    class="text-yellow-500 hover:text-yellow-700 mr-4">
                                    <i class="ri-pencil-line"></i> Edit
                                </a>
                                <form action="{{ route('admin.data_luas_lahan_produksi_pertanian.destroy', $data->id) }}"
