...
@section('page-title', 'Tambah Data Pertanian Baru')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_luas_lahan_produksi_pertanian.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="komoditas" class="block text-sm font-medium text-gray-700">Komoditas</label>
                <input type="text" name="komoditas" id="komoditas" value="{{ old('komoditas') }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="produksi" id="produksi" value="{{ old('produksi') }}"
                        required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
            </div>
