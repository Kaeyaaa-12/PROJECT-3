...
@section('title', 'Tambah Data APS')
@section('page-title', 'Tambah Data APS Baru')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_aps.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="persentase_sd" id="persentase_sd"
                        value="{{ old('persentase_sd') }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    @error('persentase_sd')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="persentase_smp" id="persentase_smp"
                        value="{{ old('persentase_smp') }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="persentase_sma" id="persentase_sma"
                        value="{{ old('persentase_sma') }}" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
