...
@section('page-title', 'Manajemen Data Pendidikan')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <a href="{{ route('admin.data_pendidikan.create') }}"
            class="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 mb-6">
            <i class="ri-add-line mr-2"></i>Tambah Data Pendidikan
        </a>
            ...
            <table class="min-w-full bg-white border">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="py-3 px-4 uppercase font-semibold text-sm text-right">Angka Melek Huruf (%)</th>
                ...
                <tbody class="text-gray-700">
                    @forelse($dataPendidikan as $data)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="py-3 px-4">{{ $loop->iteration }}</td>
                            <td class="py-3 px-4">{{ $data->kecamatan }}</td>
                            <td class="py-3 px-4 text-right">{{ number_format($data->angka_melek_huruf, 2) }}</td>
                            <td class="py-3 px-4 text-center">{{ $data->tahun }}</td>
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('admin.data_pendidikan.edit', $data->id) }}"
                                    class="text-yellow-500 hover:text-yellow-700 mr-4">
                                    <i class="ri-pencil-line"></i> Edit
                                </a>
                                <form action="{{ route('admin.data_pendidikan.destroy', $data->id) }}" method="POST"
                                    class="inline-block"
