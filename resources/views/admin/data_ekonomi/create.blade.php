...
@section('page-title', 'Tambah Data Ekonomi Baru')

@section('content')
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('admin.data_ekonomi.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="pdrb_miliar" id="pdrb_miliar" value="{{ old('pdrb_miliar') }}"
                    required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('pdrb_miliar')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="laju_pertumbuhan" id="laju_pertumbuhan"
                    value="{{ old('laju_pertumbuhan') }}" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
